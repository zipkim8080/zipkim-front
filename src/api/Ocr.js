...
function ocrParse(responseData) {
  ...
  for (const image of responseData['images']) {
    ...
    for (const table of tables || []) {
      ...
      // 첫번째 셀 텍스트라인 추출
      const firstLine = cellTextLines.length > 0 ? cellTextLines[0] : null;
      const firstCellWord = firstLine
        ? firstLine['cellWords'][0].inferText
        : '';

      // '갑구', '을구' 구역 시작 확인
      if (firstCellWord.includes('갑구') || firstCellWord.includes('갑')) {
...
function parse_gabgoo_table(table, gabgooList) {
  ...
  for (let i = 0; i < cells.length; i++) {
    ...
    if (cell['columnIndex'] === 1) {
      ...
      } else if (text.includes('압류등기말소') && gabgooList.includes('압류')) {
        gabgooList.splice(gabgooList.indexOf('압류'), 1);
      } else if (text.includes('신탁등기말소') && gabgooList.includes('신탁')) {
      ...
      if (firstWord && !isNaN(firstWord.inferText.replace(' ', '-'))) {
        ...
        const targetColumnIndex = table.cells.findIndex(
          (cell) =>
            ...
            cell.cellTextLines[0].cellWords[0].inferText.includes('근저당권')
        );
        ...
        if (targetColumnIndex !== -1 && amountIndex !== -1) {
          const proAmount = parseInt(
            table.cells[amountIndex].cellTextLines[0].cellWords[1].inferText
              .trim()
              .replace('금', '')
              .replace('원', '')
              .replace(/,/g, ''),
      ...
      if (firstWord && !isNaN(firstWord.inferText.replace(' ', '-'))) {
        ...
        const targetColumnIndex = table.cells.findIndex(
          (cell) =>
            ...
            cell.cellTextLines.length > 0 &&
            cell.cellTextLines[0].cellWords.length > 0 &&
            cell.cellTextLines[0].cellWords[0].inferText.includes('전세권')
        );
  ...
  for (const cell of table.cells) {
    if (cell.columnIndex === 1 && cell.cellTextLines.length > 0) {
      ...
      const cellTextWord = cell.cellTextLines
        .map((line) => line.cellWords.map((word) => word.inferText).join(''))
        .join('');
      ...
      if (cellTextWord.includes('말소')) {
        ...
        if (allNumbers) {
          ...
          for (let i = allNumbers.length - 1; i >= 0; i--) {
            const targetNumber = allNumbers[i].match(/(\d+)/)[1].trim();
